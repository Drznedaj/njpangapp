...
export class LoginButtonComponent implements OnInit {
  ...
  loginUrl = 'api/login';
  authToken: string;
  constructor(private loginService: LoginService, private data: DataService, private http: HttpClient) { }
  ...
  onClick() {
    if (this.userToLogin) {
     ...
     this.loginService.loginujUsera(this.userToLogin).subscribe(user => this.x = user);
